...
The block is written by `repo_setup.py`, which is the only tool that writes it: `apply`/`refresh --diary-tier|--diary-nickname|--diary-description` at setup time, and `repo_setup.py diary` (`--list`, or `--repo <path> --tier ...`) as a sweep over every repo in the worklog pot, driven conversationally by `/diary`. Because the kit is one self-contained file per tool, `repo_setup.read_diary_block` duplicates `diary_agent.repo_diary_cfg` rather than importing it, and `tests/test_repo_setup_diary.py` compares the two directly on the same inputs. Only the codename may be generated (`--nickname auto`, from a fixed list, never derived from the repo); the tier and the description are always the operator's, per ADR-0011.
