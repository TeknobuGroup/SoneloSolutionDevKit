...
- UAT-1.20-1: Add a linked worktree to a repo that has the worklog installed, collect in both, and render. The window's commit total is the repository's real commit count, not twice it. Before 1.20 every commit in that repository was counted a second time, including ones made long before the worktree existed.
- UAT-1.20-2: The same for a plain subdirectory of a checkout that has no `.git` of its own - `git -C` walks up to the parent, so it reported the parent's whole history. This is the case a "is this a worktree?" flag cannot see, and on this machine it is `PuppyParent/my-puppy-pal`: 34 commits, all of them the parent's.
- UAT-1.20-3: The fix measured on its own, on this machine on 2026-09-14, before the fetch sweep: the 28-day window read **1,772 commits, not 2,632**; W34 **467, not 715**; W35 **530, not 835**; W36 **438, not 691**; W37 **334, not 388**; flexiform-shutter-vercel's W37 **236, not 272**; Knecta's W37 **84, unchanged** (its duplicate sits outside that week); `nurture-loop-codex` out of W37 entirely, from 18. **Do not check those against the weekly CSVs** - the all-repo fetch sweep ran the same day and added commits this machine had never collected, so the numbers are correct as a record of the fix and stale as a reading of the pot.
- UAT-1.20-3a: What the CSVs read now, after the sweep, the stash fix and the re-render: the window **2,084**, where the same slices without the fix would say **3,212** (**1,128** duplicates); W34 **521**, W35 **595**, W36 **493**, W37 **468**. (An earlier draft of this case said 2,082 / 1,130 / W36 495, which was the render taken before the stash fix landed; the figures here are the ones the CHANGELOG and the re-test section carry.) The way to check the fix on live data is the difference, not the absolute: sessions, hours, tokens and cost are unchanged everywhere, and only commit counts move.
- UAT-1.20-4: `nurture-loop-codex` no longer appears in the projects table as a project of its own. Its commits belong to the `nurture-loop-tek` repository and count under Knecta; with no sessions of its own it lists as quiet.
- UAT-1.20-5: `PuppyParent/my-puppy-pal` is still listed, with **0 commits and its 1 session kept**. A second checkout is a real place work happened; only its commits are another slice's to report, and its uncommitted count stays its own.
- UAT-1.20-6: Two clones of *different* repositories are never merged, even when their first commit happens to have the same hash - which it does whenever both were seeded with the same content, message, author and second. A slice that knows its own repository is never folded into another on the strength of a coincidental hash.
- UAT-1.20-7: A slice that stopped being collected keeps contributing the history only it holds. `nurture-loop-codex` is frozen at 7 Sep and carries 24 commits from 11-14 Aug that Knecta's live slice has already dropped past its window; those still appear, exactly once. The group's commits are unioned by hash, not taken from whichever slice was picked.
- UAT-1.20-8: The projects table on the dashboard and the Summary total in the weekly report agree on the commit count for the same window. They are computed on the two sides of the Python/JavaScript split, and the dashboard payload is deduplicated before it ships so the flatten inherits the correction.
- UAT-1.20-9: The diary's day-file does not double-count either. For a checkout marked as a duplicate, `diary_agent` stops asking git directly - which it does for any path still on disk - and reports that repository's commits once, under the checkout at the repository root. Its sessions still count.
- UAT-1.20-10: The slice **files** are untouched. `status` still shows the raw per-file numbers, because it describes files rather than work, and re-collecting does not rewrite history - the correction applies to slices already on disk, not only to what is collected from now on.
- UAT-1.20-11: `status` reports worklog 1.20 and the session-start message names what changed: commits counted once per repository.

### Fixed: malformed slice files no longer break the pot loader

- UAT-1.20-12: Create a file `<pot>/slices/corrupt.json` containing a JSON array instead of an object: `[{"project": "X"}]`. Run `render`. Verify that `weekly/worklog-YYYY-Www.md`, `weekly/worklog-YYYY-Www.csv`, `dashboard.json`, and `morning.html` are all still produced and contain correct counts for every other slice in the pot. The corrupt file is silently skipped; its absence costs that file and nothing else.
- UAT-1.20-13: In an existing slice file, modify the `commits` array to include malformed entries: add a string `"not a commit"` and a number `42` alongside valid commit objects. Run `render`. Verify that the slice's valid commits are still counted, the invalid entries are ignored, and the slice's count matches what it would be if those entries were removed. Weekly report, CSVs, dashboard and morning page all produced with correct totals.
- UAT-1.20-14: In a slice, modify a commit object to have `"hash": {}` (JSON object instead of string) and another with `"hash": []` (JSON array), leaving both with their `time` and `subject` intact. Run `render`. Verify that `render` completes and that **both entries are still counted** - they appear in the day's list with an empty hash where the short hash usually is, and the slice's total is unchanged. That is deliberate: the entry's time and subject are good, so it is real work, and dropping it would take a commit out of the count to remove a duplicate that was never there. What must NOT happen is a traceback, or a blank/partial `dashboard.json`.
- UAT-1.20-15: Create or modify two slices, **neither carrying a `repo_id`**, such that one is corrupt (its `commits` array contains non-dict entries mixed with valid dicts) and both contain the same **two** commit hashes, `"abc123def456"` and `"def456abc123"`. Two shared hashes are the threshold - one is a coincidence and is deliberately not acted on (see UAT-1.20-16). Run `render`. Verify that each of those two hashes is counted exactly once in the output, not twice, and that every other commit from both slices is still counted.
- UAT-1.20-16: Repeat UAT-1.20-15 with the two slices sharing **one** hash only. Run `render`. Verify the opposite: both slices keep their own commits, neither is marked as a duplicate of the other, and the shared hash is counted twice. Then check `<pot>/worklog.log` and confirm no `dedupe: merged ...` line was written for them. This is the guard against one coincidental root commit absorbing a whole project - two repositories started from the same template share their first commit exactly.
- UAT-1.20-17: In a repo the worklog collects, run `git stash push -u -m "uat"` with both a modified tracked file and an untracked file present, then `run --sync` and `render`. Verify that no entry titled `index on <branch>: ...` or `untracked files on <branch>: ...` appears in the weekly report, the CSV or the dashboard's log, and that the repo's commit count is unchanged by stashing. Then `git stash pop` and commit the change for real, re-run, and verify it now appears exactly once.
- UAT-1.20-18: In any slice in the pot, replace `"sessions": [...]` with `"sessions": "x"`, and in a second slice set `"uncommitted": "lots"`. Run `render`. Verify every other project still appears in `latest.md` with its usual commit and session counts, the dashboard still rewrites, and `.worklog/agent.log` names both files as having had a field of the wrong type. Repeat with `"project": 1` and verify that slice alone is absent from the report and named in the log as dropped.
- UAT-1.20-19: In a repo the worklog collects, create a branch whose name contains markup - `git switch -c 'x<img src=q onerror=alert(1)>'` - commit on it, then `run --sync` and `render`. Open `dashboard.html` and verify the branch name is shown as text beside the project heading, no dialog appears, and nothing is missing from the page.
- UAT-1.20-20: In a repo the worklog collects, run `git notes add -m "a note"` on the most recent commit, then `run --sync` and `render`. Verify the commit count for that repo is unchanged and no entry reading `Notes added by 'git notes add'` appears in `latest.md`, the day list on the dashboard, or `diary collect` for that day.
- UAT-1.20-22: In one slice, set a session's `"active_min"` to `"lots"`; in a second, set a session's `"bursts"` to `5`; in a third, set `"tools"` to `"x"`. Run `render` after each. Verify every other project still appears in `latest.md` with its usual counts, the dashboard still rewrites, and `.worklog/agent.log` names the file as having had a field of the wrong type. Then set a session's `"tokens"` to `{"in": "lots"}` and verify the same, with that session's token total reading zero rather than stopping the report.
- UAT-1.20-23: Open `<pot>/slices/_machine__<name>.json` and replace `"aw"` with `"x"`, then run `render`. Verify the weekly report, the CSVs, the dashboard and the morning page are all still written, with desk-time figures absent rather than the whole report missing. Repeat with `"presence": "none"`, and with a presence event that has no `"time"` key. Restore the file afterwards - this is the machine's own desk-time record and nothing rebuilds the history in it.
- UAT-1.20-24: With any of the malformed files above still in place, run `python .worklog/worklog_agent.py status`. Verify it prints the slice line, the machine line and the log path without a traceback. This is the command someone runs *because* the pot looks wrong, and it reads the files raw on purpose.
- UAT-1.20-21: With a repo on disk that has no `user.name` or `user.email` set and no work at all on the chosen day, run `python diary_agent.py collect <date>`. Verify that repo is not named in the day's warnings. Then make a commit in it on that day, re-run, and verify the warning about every author's commits does appear.
- UAT-1.20-25: Take a slice for a repo that is **not on this machine** (a path that does not exist - the machine-to-machine import produces these), make sure `diary.json` has no `authors` list, and give it a commit dated on the chosen day by somebody other than you. Run `python diary_agent.py collect <date>`. Verify the warnings say that repo's commits could not be filtered by author and that every author's commits are in the day - not only that they carry no file counts.

**Pushed to UAT Hub** on 2026-09-14 as module `07. Worklog - a commit is counted once`:
18 cases, into round 1, which the hub reported back as 123 cases across 7 modules. They are
written for a tester with no sight of the code, so they are ordered to run top to bottom -
case 01 backs the pot up before cases 13-16 damage it, and case 18 puts it back.

### Re-test required: anything that quoted a commit count

- **Machine-to-machine slice import guard**: UAT-1.20-12 through UAT-1.20-15 verify that `load_slices()` is resilient to hand-edited or corrupted `.json` files copied between machines. This hardening is the safety net for slice imports, which bypass the normal collection process. If a slice file is accidentally malformed during transfer, the pot loader skips it and continues rendering rather than failing with `TypeError` on unhashable types.

- **Every commit figure recorded before 1.20 is too high**, by 860 across this machine's 28-day window as it stood on 14 September, and by 1,128 once the fetch sweep had filled the same slices in. `render()` rewrites each `weekly/worklog-YYYY-Www.md|.csv` in the window on every run, so the four weeks inside it restate themselves downward on the first render, and the Weeks chart moves with them. Weeks **older** than the window are never re-rendered and keep their inflated numbers permanently - nothing recomputes them, and that is disclosed rather than fixed.
- **UAT-1.18-3**: the agent-hours invariant is unaffected - hours, sessions, tokens and cost never went through the duplicated lists. The sweep has since run (18 repos, 2026-09-14), so this is due rather than anticipated: it is the first render in which every repo's slices were rewritten at once.
