...
Kit v4.9 · worklog v1.19. Sonelo is the developer platform brand of Teknobu Group. Repo standards, a complete agent pipeline, infrastructure creation and a work log for Claude Code projects. One Python file each, standard library only, Windows first.
...
**A diary of the day, safe to publish from.** `diary_agent.py` gathers one day - commits, worklog entries, hours, Claude Code session summaries, and the notes and photos you added with `diary note` - into `~/Diary/<date>.json`, and serves it to Claude over a local MCP server (`diary serve`, four tools, registered in the desktop app). Every repo is **private until you say otherwise**: with no `diary` block in its `.teknobu.json` it contributes hours and nothing else. Raise it per repo to `nickname` (feature-level narrative, no commit messages, paths, branches, URLs or people) or `own` (full detail about the work - still never the product's name). Before a day-file is written, every string in it is checked against a blocklist, the repo and person names in the day, and, below `own`, the shapes that identify a client anyway; a hit **fails the collect** and names the term and the field it came from, rather than redacting it quietly (`--force` redacts and records what it did). Session summaries are generated on your machine from prompts and prose only - never tool results - and are the only session content that reaches the file: raw transcripts never leave the machine. See `DIARY.md`.
