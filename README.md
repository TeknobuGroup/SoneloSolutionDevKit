...
Kit v4.14 · worklog v1.19 · diary v1.0. Sonelo is the developer platform brand of Teknobu Group. Repo standards, a complete agent pipeline, infrastructure creation and a work log for Claude Code projects. One Python file each, standard library only, Windows first.
...
**A complete pipeline, built in.** Eleven agents: impact-analyst (before), Explore (read-only search), code-reviewer, security-reviewer and design-reviewer (read-only, in parallel), test-writer (the only writer, tests only; failing test first for every bug), test-runner, qa-runner (Playwright against the work-branch URL), uat-writer, changelog-scribe and docs-maintainer (Haiku). Three commands: `/post-change` (review → fix loop, two rounds max → tests → verdict → docs), `/design-pass` (design-led polish within the contract, nothing that touches logic), `/pr` (UAT document required, becomes the PR body). Three Claude Code hooks: typecheck and lint after every edit, migrations append-only, and a Stop gate that holds the session until the changelog entry, regenerated types and a clear review verdict exist. CI gates mirror it on every PR: changelog, UAT document, types - and a migration that genuinely cannot change types (policy, grant or data only) passes the types gate by saying so in a commit trailer, `Types-not-affected: <reason>`, which CI prints in the log for the reviewer - a declaration on the record, not a check. The lead is the session you type into; the agents are its specialists. A starter folder of your own seeds any file the kit does not own; the files the kit does own it restores to the current release on `refresh` (or `apply --update-pipeline`), keeping a backup in `.claude/.backup/`. `refresh` is the narrow verb: agents, commands, hooks, the kit's CI gates and the `uat-hub` entry in `.mcp.json` - never the repo's own ci.yml, env files, design contract or branches.
...
**UAT that a human actually sees.** Every repo comes wired to [UAT Hub](https://testing.teknobugroup.com): `.mcp.json` registering the `uat-hub` MCP server (merged into any you already have), a managed "Writing UAT" section in `CLAUDE.md` telling sessions to push test cases to the hub instead of writing a Markdown file nobody opens, `UAT_HUB_KEY` documented in `.env.example`, and a `doctor` line saying whether that variable is set. The key itself is never written to a file - `.mcp.json` holds the `${UAT_HUB_KEY}` placeholder and is expanded from your environment, because `.mcp.json` is committed and one key covers every project. The server path is a variable too - `${USERPROFILE}/uat-hub/mcp/server.mjs` on Windows, `${HOME}/...` on Mac and Linux, both accepted everywhere - so the committed file names no machine and no username. The project slug is asked for once and remembered; the project must already exist in the hub, since a push is refused rather than creating one, which is what stops a typo inventing a phantom client.

**Lovable migrations are a clean break.** New GitHub repo, new Supabase project, new Vercel project, and nothing of Lovable's kept - because Lovable's `src/integrations/supabase/client.ts` hardcodes the project URL and key, so a repo can be moved everywhere else and still read Lovable Cloud on every request, with nothing failing to say so. Detected from the code, `MIGRATION.md` is written around three sweeps: the three new instances, every external connection with a keep/replace/drop decision on each (named where the kit recognises the host, marked **unrecognised** where it does not - never dropped), and the SEO and branding rewrite Lovable leaves behind in `index.html`, the favicons, the Open Graph image and `package.json`. `repo_setup.py lovable` re-runs that sweep any time; `--strict` exits non-zero while anything still ties the repo to Lovable, so the cutover is gated rather than declared. It reads env files for key *names* only - the one value it ever reports is the Supabase project ref, which is public and sits in the dashboard URL. ADR-0010.

**A diary of the day, safe to publish from.** `diary_agent.py` gathers one day - commits, worklog entries, hours, Claude Code session summaries, and the notes and photos you added with `diary note` - into `~/Diary/<date>.json`, and serves it to Claude over a local MCP server (`diary serve`, four tools, registered in the desktop app). Every repo is **private until you say otherwise**: with no `diary` block in its `.teknobu.json` it contributes hours and nothing else. Raise it per repo to `nickname` (feature-level narrative, no commit messages, paths, branches, URLs or people) or `own` (full detail about the work - still never the product's name). The kit asks for the tier when it sets a repo up (`apply`/`refresh --diary-tier`), and `repo_setup.py diary --list` - or `/diary` in a session - walks every repo the worklog knows about so an estate can be classified in one sitting. Only the codename is ever generated: the tier and the description stay yours, because a tier inferred from a folder name is a classification nobody made. Before a day-file is written, every string in it is checked against a blocklist, the repo and person names in the day, and, below `own`, the shapes that identify a client anyway; a hit **fails the collect** and names the term and the field it came from, rather than redacting it quietly (`--force` redacts and records what it did). Session summaries are generated on your machine from prompts and prose only - never tool results - and are the only session content that reaches the file: raw transcripts never leave the machine. See `DIARY.md`.
