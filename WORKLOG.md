...
- How the time columns differ: the dashboard shows both, side by side. *agent-hours* is effort - each session's own active time, summed, so five sessions working an hour each is five agent-hours. *elapsed* (and the *Claude Code (elapsed)* column on a day row) is wall-clock time with any session active, so those same five concurrent sessions count once. The per-project figures are effort and match the agent-hours card, which is why they can add up to more than the day. Both use gaps between transcript events capped at `idle_minutes` (15). *Editor* is the VS Code window in front while keyboard or mouse were active; *at desk* is all keyboard/mouse activity in any app; *unlocked* is between a logon/unlock stamp and lock/sleep/shutdown, so it only exists from the day the presence tasks were registered (boot and wake events from the event log don't count on their own, because Windows wakes itself for updates). None is a billable figure; together they show the shape of the day.
